...
for x in range(iterations):
    ...
    def fio_async_command_generator(option):
        global fio_command
        fio_command = ['fio',option,fio_filename,"-bs=128k","-size=128M",fio_numjobs,fio_runtime,fio_direct,'-iodepth=32','-ioengine=libaio','-time_based','--output-format=json','--output=fio.json','-time_based' ,'-group_reporting','-exitall']
        return fio_command
